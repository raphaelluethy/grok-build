...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build grog"))?;

    if !output.status.success() {
        bail!(
            "failed to build grog (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_grog` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-grok-pager-bin` (the composition-
///    root package that owns the `grog` binary)
