...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("grog{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "xai-grok-pager-bin", "--bin", "grog"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build grog: {e}"));

    assert!(
        output.status.success(),
        "failed to build grog for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "grog build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve CLI binary: `GROK_BINARY` env (CI) or a locally built `grog` binary.
pub fn grok_binary() -> PathBuf {
    ...
    if let Ok(path) = std::env::var("CARGO_BIN_EXE_grog") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
