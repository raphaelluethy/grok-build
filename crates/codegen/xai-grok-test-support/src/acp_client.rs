...
/// Spawn `grog agent stdio` with the canonical hermetic test env: the sandbox
...
/// Drives `grog agent stdio` via the ACP protocol over pipes.
///
...
/// Drives `grog agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
