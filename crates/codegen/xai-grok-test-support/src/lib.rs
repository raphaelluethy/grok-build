...
//! - [`GrokStdioClient`] — ACP client that drives `grog agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `grog agent --leader stdio` (unix)
//! - [`run_headless`] — Run `grog -p` against the mock server and capture output
