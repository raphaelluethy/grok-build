//! `grog inspect` — configuration introspection.
//!
...
/// A single config layer entry for `grog inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
fn list_lsp_servers(
    ...
    // `grog mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
fn print_human(r: &InspectReport) {
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `grog mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
