...
/// files (TUI tick vs `grog login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `grog setup`; runs retries to completion.
    Standard,
...
/// Callers doing auto-fetch should check this; explicit user actions (grog setup) skip it.
...
struct SyncOutcome {
    ...
    /// `grog setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// read-only `grog setup --json` both build on this.
...
/// Post-login hook for `grog login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `grog setup` could install config for.
...
/// `grog setup`); ceasing to serve `fail_closed` rolls back.
...
/// Outcome of the `grog setup` sync. The caller renders it — CLI presentation
...
/// Result of `grog setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `grog setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `grog setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `grog setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `grog setup` would refuse
...
/// Run the `grog setup` sync for the current principal. The caller must check
