//! `grog mcp doctor` -- runtime health check for MCP servers.
...
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `grog login`"),
    };
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `grog mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `grog mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `grog mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `grog mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
