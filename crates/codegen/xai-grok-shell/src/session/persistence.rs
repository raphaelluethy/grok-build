...
/// `grog -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `grog -r <remote_id>` before this fix was deployed.
        ...
        StorageMode::Writeback => {
            ...
            let auth_manager = auth_manager.ok_or_else(|| {
                io::Error::new(
                    ...
                    "Writeback storage mode requires authentication. Run 'grog login' first.",
                )
            })?;
...
/// worktree containing this session's cwd so `grog worktree gc` expires by
...
mod find_local_child_tests {
    ...
    /// Regression: a second `grog -r <remote_id>` must return the existing child
