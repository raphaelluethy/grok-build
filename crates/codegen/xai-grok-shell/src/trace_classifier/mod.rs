...
pub async fn resolve_api_key(explicit: Option<&str>, grok_home: &Path) -> Result<String> {
    ...
    Err(anyhow!(
        "no API key: pass --api-key, set XAI_API_KEY, or run `grog login` to populate \
...
async fn non_interactive_auth_key(grok_home: &Path) -> Result<Option<String>> {
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `grog login` to re-authenticate, \
...
mod tests {
    ...
    /// OIDC scope (the same scope `grog login` writes today and
