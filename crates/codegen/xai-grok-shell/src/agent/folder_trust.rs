...
/// whose cwd differs from the launch dir, `grog mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `grog --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `grog mcp doctor`). It MUST
...
mod tests {
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `grog --trust` grant landing in the store after the
