...
impl EndpointsConfig {
    ...
    /// Managed deployment-config URL (`grog setup`): explicit `managed_config_url`,
...
/// (`grog workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `grog workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `grog inspect`.
    #[serde(skip)]
...
/// by `grog setup`) sit under the env vars, requirements pins are applied on
