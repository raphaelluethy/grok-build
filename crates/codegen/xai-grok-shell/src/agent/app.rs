...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by grok-desktop when spawning `grog agent stdio`).
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `grog agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_grok_telemetry::unified_log::set_version(xai_grok_version::VERSION);
    // `grog agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `grog agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `grog login` to sign in, or use `grog agent stdio` for API-key access.";
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'grog login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `grog login`."),
        }
...
///
/// * `relay_on_demand == false` (default — explicit `grog agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `grog agent leader` (devbox / systemd) has no local IPC clients
                ...
                while let Some(update) = config_update_rx.recv().await {
                    match update {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            ...
                            // (another grog process fetched a fresher /v1/models
...
mod tests {
    ...
    /// `grog agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `grog agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
