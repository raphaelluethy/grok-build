...
use crate::agent::session_config::{CONFIG_ID_FAST_MODE, CONFIG_ID_MODEL, CONFIG_ID_REASONING_EFFORT};
    ...
    match config_id.0.as_ref() {
        ...
        CONFIG_ID_FAST_MODE => {
            let enabled = value.as_bool().ok_or_else(|| {
                acp::Error::invalid_params().data("fast_mode requires a boolean value")
            })?;
            agent.set_session_fast_mode(&session_id, enabled)?;
        }
        other => {
            return Err(
