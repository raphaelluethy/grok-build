...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `grog login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grog login`, set XAI_API_KEY, or add api_key to ~/.grok/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `grog login` to authenticate.";
...
mod tests {
    ...
    // -- grog login --legacy regression coverage ------------------------
    //
    // `grog login --legacy` produces a GrokAuth with `auth_mode: WebLogin`,
    ...
    fn grok_login_legacy_token_does_not_require_login() {
        ...
        // Construct a legacy-style token exactly as `grog login --legacy`
