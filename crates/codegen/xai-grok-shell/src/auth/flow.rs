...
async fn run_auth_flow_inner(
    ...
    // `grog login --devbox` uses run_devbox_login and is not gated here.
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `grog login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
/// Unified `grog login` handler for CLI entry points (tui, pager).
///
...
pub async fn run_cli_login(
    ...
    // supports the device flow. Without this guard, `grog login` on an
    ...
    // config was actually applied; `grog setup` reports the no-config case.
...
/// `grog logout` CLI handler. Calls [`perform_logout`] and formats
...
mod tests {
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `grog login` must use loopback, not device —
    ...
    /// CLI `grog login` passes `on_stderr=None`; stderr must be inherited so
