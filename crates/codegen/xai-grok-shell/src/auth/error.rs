...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `grog login`.")]
    NotLoggedIn,

    /// Token expired and no refresh authority available.
    #[error("Token expired. Run `grog login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `grog login` to re-authenticate.")]
    ...
    #[error("{message} Run `grog login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// Cached API-key session rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `grog login` to authenticate.")]
    ApiKeyAuthDisabled,
    ...
    pub(crate) fn user_message(self) -> &'static str {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `grog login` to sign in again."
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `grog login` if this persists."
            }
            Self::Other => {
                "Authentication could not be refreshed. Run `grog login` to sign in again."
            }
        }
    }
}
