...
/// auth.json scope key for plain API key auth (desktop login, `grog login --api-key`).
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from grok-desktop login or `grog login --api-key`)
    ApiKey,
}
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `grog login --legacy`
