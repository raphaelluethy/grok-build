...
pub struct LeaderCapabilities {
    ...
    /// (driven by `grog update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `grog update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `grog update` just installed; the leader uses
