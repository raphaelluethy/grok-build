...
/// Maximum reconnection attempts for bounded mode (headless/`grog -p`).
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`grog -p` where hanging forever is unacceptable.
...
/// (e.g. `~/.grok/...`) — prefer the managed `~/.grok/bin/grog` symlink. After an
/// auto-update or `grog update` atomically swaps that symlink, `current_exe()`
...
/// calling binary.
///
/// Falls back to `~/.grok/bin/grog` only when `current_exe()` is unavailable.
...
/// Binary file name for the managed grok install (`grog` / `grok.exe`).
...
mod tests {
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("grog"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("grog")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
