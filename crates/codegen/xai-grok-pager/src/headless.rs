//! Headless single-turn mode (`grog -p "prompt"`).
//!
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `grog login` to authenticate \
         (or `grog login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         grog login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `grog login` on a machine with a browser."
            .to_string()
    }
}
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'grog models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    // hub_url lives on `grog workspace` / WorkspaceStartArgs only).
