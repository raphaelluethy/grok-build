//! `grog wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `grog ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`grog wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("grog wrap: no command given"))?;
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("grog wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `grog wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`grog wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`grog wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `grog ssh`, `grog wrap` does not gate on the terminal brand: the user
