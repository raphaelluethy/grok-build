//! `grog plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
/// Typed entry for `grog plugin list --json`. The `status` field acts as a
...
/// Typed entry for `grog plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `grog plugin list`).
        name: String,
...
fn cmd_list(json: bool, available: bool) -> Result<()> {
    ...
    if json {
        ...
        println!("No plugins installed. Run `grog plugin install --help` to get started.");
    } else {
            ...
            if outcome.already_installed {
                ...
                println!(
                    "Plugin \"{}\" is already installed from {}. \
                     Run `grog plugin update {}` to update it.",
    ...
    if registry.find_plugin(name).is_none() {
        bail!(
            "Plugin \"{name}\" not found.\n\
               Run `grog plugin list` to see installed plugins."
        );
    }
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `grog plugin tag`."
            )
        })?,
    ...
    if json {
        ...
        println!(
            "No marketplace sources configured.\n\
             Run `grog plugin marketplace add --help` to get started."
        );
    } else {
        for s in sources {
            let id = match &s.kind {
