//! `grog completions <shell>` — generate shell completion scripts.
//!
...
/// `$line[2]` empty, and the dispatch falls through — so `grog worktree <TAB>`
...
mod tests {
    ...
    // so `grog worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
