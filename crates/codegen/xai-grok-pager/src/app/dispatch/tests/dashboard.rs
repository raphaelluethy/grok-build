...
/// `grog dashboard` before login: the startup hook consumes the
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `grog dashboard` must open the dashboard after login",
    );
    assert!(
...
/// `grog dashboard`, and the startup hook.
#[serial_test::serial(GROK_AGENT_DASHBOARD)]
#[test]
