...
```bash
grog --resume <session-id>
```

Run `grog --resume` without an ID to resume the most recent session for the current directory.

### From the Welcome Screen

When you launch `grog`, the welcome screen lists recent sessions for the current directory. Select one to resume it.
...
grog -p "Continue where we left off" -r <session-id>
...
grog sessions list --limit 50

# Search sessions by keyword (matches titles and prompts)
grog sessions search "rate limit"
```

`grog sessions list` shows sessions for the current working directory, grouped by worktree label. Each row lists the session ID, the creation and update dates, the source status, and the summary. `grog sessions search` combines a local SQLite index with remote results.
...
Resume a session in a fresh worktree with `grog -w -r <session-id>`.
...
The smaller state files -- `summary.json`, `plan.json`, and `signals.json` -- are plain JSON rather than JSONL. JSONL is the source of truth for session content; `grog sessions search` additionally maintains a local SQLite FTS5 index over session titles and prompts for fast keyword search.
