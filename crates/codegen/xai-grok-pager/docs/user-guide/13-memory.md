...
The `grog memory` command manages memory from the shell. It has one subcommand, `clear`:
...
1. Verify memory is enabled: check `grog inspect` output.
2. Check the flag: `grog --experimental-memory` or `GROK_MEMORY=1`.
