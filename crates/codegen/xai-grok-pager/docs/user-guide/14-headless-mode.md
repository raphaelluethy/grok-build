...
By default, each `grog -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
grog -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
grog -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
grog -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
grog -p "What's the secret number?" --resume abc123
```
...
grog -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
grog --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  grog -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
RESULT=$(grog -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
grog -p "Format all files" --yolo

# Run tests and fix failures
grog -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
grog -p "Run the test suite" --yolo
```
...
- **`grog login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`grog login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `grog -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug grog -p "..." 2> debug.log`.
...
- Resume: `grog -p "continue" --resume "<id>"` or `grog -p "continue" --continue`
