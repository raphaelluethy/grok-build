...
To browse your skills, type `/` to open the slash-command menu. Grok lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `grog inspect` (see [Viewing Skill Details](#viewing-skill-details)).
...
Run `grog inspect` to see every skill Grok discovers, along with the rest of your configuration:

```bash
grog inspect          # Human-readable summary
grog inspect --json   # Machine-readable report
```
