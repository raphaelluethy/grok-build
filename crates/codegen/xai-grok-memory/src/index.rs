...
impl MemoryIndex {
    ...
    /// a stale one).  Used by `grog memory doctor` to detect stuck states.
    ...
    ///
    /// Used by `grog memory doctor` to detect orphaned chunks (chunks whose
...
mod tests {
    ...
    /// 3. Run the same orphan-removal logic as `grog memory reindex`:
    ...
    ///
    /// This proves that `grog memory reindex`'s Phase 1 actually fixes the
    /// state that `grog memory doctor` warns about.
    #[test]
    fn test_reindex_maintenance_removes_orphaned_chunks() {
        ...
        // Simulate `grog memory reindex` Phase 1: compare indexed vs current.
    ...
    ///
    /// Verifies that `grog memory reindex` Phase 0 correctly bails when a
    ...
    /// `grog memory reindex` Phase 3 resets the stale reindex claim.
    ///
    /// Verifies that `release_claim()` clears `meta.reindex_claim` so that
    /// `grog memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
    fn test_reindex_maintenance_resets_stale_claim() {
        ...
        // Simulate `grog memory reindex` Phase 3: release the claim.
        idx.release_claim();
