...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    let symlink = home.join("bin").join("grog");
    ...
    // `grog` and `agent` move together — see `swap_managed_bin_links`.
...
async fn install_internal_updates_stale_agent_symlink_to_new_version() {
    ...
    std::os::unix::fs::symlink(&rel_old, bin_dir.join("grog")).unwrap();
...
/// Rollback regression: if `agent` swap fails after `grog` succeeded,
/// `grog` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_grok_when_agent_swap_fails() {
    ...
    let grok_target = std::fs::read_link(bin_dir.join("grog")).unwrap();
    assert_eq!(
...
/// Absent-prior rollback regression: fresh install (no prior `grog` /
/// `agent`), sabotaged `agent` swap must *remove* the just-created `grog`
...
async fn install_internal_rollback_removes_absent_prior_grok_link() {
    ...
    // No prior `grog`. Sabotage `agent` swap: non-empty directory → EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("grog").exists() && !bin_dir.join("grog").is_symlink(),
    ...
    let grok_path = bin_dir.join("grog");
    assert!(
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    let target = std::fs::read_link(home.join("bin").join("grog")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("grog")).unwrap();
