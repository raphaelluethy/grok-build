...
async fn internal_install_stable_upgrade_0_2_5_to_0_2_7() {
    ...
    let symlink = test_home().join("bin").join("grog");
    let target = std::fs::read_link(&symlink).unwrap();
...
// explicit `grog update`) must decide staleness from the on-disk install, not
...
async fn npm_user_upgraded_then_stable_rolled_back_stays_on_newer() {
    // User ran `grog update` and got 0.2.7. Then stable was rolled back to
