...
//! `max(latest, minimum)`; otherwise the user is asked to run `grog update`.
//!
...
pub(crate) enum MinimumVersionError {
    ...
    #[error(
        "This version of Grok ({current}) is no longer supported. \
         Run `grog update` to install version {minimum} or later."
    )]
    ...
    /// `grog update --version X` requested a version below the floor.
    #[error(
        "Cannot install Grok {target}: the configured minimum is {minimum}. \
         Run `grog update` to install the latest allowed version."
    )]
...
/// Used by `grog update` to keep the install target at or above the pin.
